		...
		public List<long> GetAllGids()
		{
			...
			try
			{
				foreach (DMSType type in Enum.GetValues(typeof(DMSType)))
				{
					...
					{
						iterID = GetExtentValues(modelResourcesDesc.GetModelCodeFromType(type), properties);
						int resourcesLeft = IteratorResourcesLeft(iterID);

						while (resourcesLeft > 0)
						{
							List<ResourceDescription> rds = IteratorNext(numberOfResources, iterID);
							...
							resourcesLeft = IteratorResourcesLeft(iterID);
						}

						bool ok = IteratorClose(iterID);
					}
				}
			}
			catch (Exception)
